...
def initiate_return(order_id: str, reason: str) -> str:
    """Initiate a return for a shipped or delivered order.
    ...
    """
    print(f'TOOL USED: initiate_return for order {order_id}, reason: {reason}')
    ...
    if success:
        ...
        if status_info['status'] == 'SHIPPED':
            ...
            next_steps = f"Please keep the {product_name} when it arrives and use the return label we'll email you."
        else:
            status = 'RETURN_INITIATED_FOR_DELIVERED_ITEM'
            next_steps = f"Please use the return label we'll email you to send the {product_name} back."
